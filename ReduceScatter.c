...
void main(int argc, char* argv[])
{
	...
	//size :  number of elements that receve from each proc {size_proc0,size_proc1,.....} ,dependent on size of receve buffer
	MPI_Gather(&recbuf,1,MPI_INT, &data,1, MPI_INT, 0, MPI_COMM_WORLD);
	//you can solve it without gather,and print the value of recbuf,but the printf statments do not work in order
